...
//  end;
 ...
 {-----------------------------------------------------------------------------}
  ...
  begin
    ...
    { Загружаем VisualCompare, если он еще не загружен }
    ...
    if not Assigned(vGetApiProc) then
     Exit; {VisualCompare неподходящей версии }
 ...
 {-----------------------------------------------------------------------------}
  ...
  begin
    ...
    if (vRes = GIT_ENOTFOUND) and FileNameIsLocal(aFolder) then begin
      { Дополнительно проверим корень диска... }
...
//  vFileName2 := UTF8ToWide(aDelta.new_file.path);
    ...
    if aRootPath <> '' then begin
      if not UpCompareSubStr(aRootPath, vFileName1) = 0 then
        {Такого не должно бытьб прверка на всякий случай... }
        Exit;
...
*)
 ...
 {-----------------------------------------------------------------------------}
  ...
  procedure TGitRepository.ShowCommit(const AFolder :TString);
    { Показывает разницу между Head и рабочим каталогом (git status) }
  var
    i, n :Integer;
    vVCAPI :IVisCompAPI;
    vStatusList :Pointer;
