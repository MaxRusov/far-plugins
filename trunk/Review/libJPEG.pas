...
{-$Define JPEG_LIB_VERSION_70}
{-$Define JPEG_LIB_VERSION_80}

const
  cJPegLibDLLName = 'jpeg62.dll';
...
Type
  ...
  JBOOL = WORDBOOL; //Byte; // Under windows they are only 1 byte large.
...
Type
  ...
  jpeg_decompress_struct = Record
    ...
    num_components: integer; { # of color components in JPEG image }
    jpeg_color_space:DWORD {J_COLOR_SPACE}; { colorspace of JPEG image }

    { Decompression processing parameters --- these fields must be set before
      calling jpeg_start_decompress().  Note that jpeg_read_header() initializes
      them to default values. }

    out_color_space:DWORD {J_COLOR_SPACE}; { colorspace for output }
   ...
   {$ifdef JPEG_LIB_VERSION_80}
    ...
    natural_order :PInteger;           { natural-order position array for entropy decode }
...
Function init_libJPEG(const APath :TString): boolean;
...
Var
  Temp: Boolean;
Begin
  If (libJPEG_RefCount = 0) Or (libJPEG_Handle = 0) Then Begin
...
//   {$ENDIF}
    ...
    If libJPEG_Handle = 0 Then
      libJPEG_Handle := LoadLibrary(cJPegLibDLLName);

    If libJPEG_Handle <> 0 Then Begin
...
Var
  ...
  function LocGetProc(AProcName :PAnsiChar) :Pointer;
  begin
    Result := GetProcAddress(libJPEG_Handle, AProcName);
...
//   {$ENDIF}
  ...
  libJPEG_Handle := LoadLibrary(PTChar(AddFileName(APath, cJPegLibDLLName)));
  if libJPEG_Handle = 0 then
    begin Result := False; exit; end;

  vAllFound := True;

  @jpeg_std_error := LocGetProc('jpeg_std_error');
  @jpeg_resync_to_restart := LocGetProc('jpeg_resync_to_restart');
  @jpeg_CreateDecompress := LocGetProc('jpeg_CreateDecompress');
  @jpeg_destroy_decompress := LocGetProc('jpeg_destroy_decompress');
  @jpeg_read_header := LocGetProc('jpeg_read_header');
  @jpeg_start_decompress := LocGetProc('jpeg_start_decompress');
  @jpeg_finish_decompress := LocGetProc('jpeg_finish_decompress');
  @jpeg_read_scanlines := LocGetProc('jpeg_read_scanlines');
  @jpeg_abort := LocGetProc('jpeg_abort');

(*
  @jpeg_CreateCompress := LocGetProc('jpeg_CreateCompress');
  @jpeg_destroy_compress := LocGetProc('jpeg_destroy_compress');
  @jpeg_set_defaults := LocGetProc('jpeg_set_defaults');
  @jpeg_set_colorspace := LocGetProc('jpeg_set_colorspace');
  @jpeg_default_colorspace := LocGetProc('jpeg_default_colorspace');
  @jpeg_set_quality := LocGetProc('jpeg_set_quality');
  @jpeg_set_linear_quality := LocGetProc('jpeg_set_linear_quality');
  @jpeg_add_quant_table := LocGetProc('jpeg_add_quant_table');
  @jpeg_quality_scaling := LocGetProc('jpeg_quality_scaling');
  @jpeg_simple_progression := LocGetProc('jpeg_simple_progression');
  @jpeg_suppress_tables := LocGetProc('jpeg_suppress_tables');
  @jpeg_alloc_quant_table := LocGetProc('jpeg_alloc_quant_table');
  @jpeg_alloc_huff_table := LocGetProc('jpeg_alloc_huff_table');
  @jpeg_start_compress := LocGetProc('jpeg_start_compress');
  @jpeg_write_scanlines := LocGetProc('jpeg_write_scanlines');
  @jpeg_finish_compress := LocGetProc('jpeg_finish_compress');
  @jpeg_write_raw_data := LocGetProc('jpeg_write_raw_data');
  @jpeg_write_marker := LocGetProc('jpeg_write_marker');
  @jpeg_write_m_header := LocGetProc('jpeg_write_m_header');
  @jpeg_write_m_byte := LocGetProc('jpeg_write_m_byte');
  @jpeg_write_tables := LocGetProc('jpeg_write_tables');
  @jpeg_read_raw_data := LocGetProc('jpeg_read_raw_data');
  @jpeg_has_multiple_scans := LocGetProc('jpeg_has_multiple_scans');
  @jpeg_start_output := LocGetProc('jpeg_start_output');
  @jpeg_finish_output := LocGetProc('jpeg_finish_output');
  @jpeg_input_complete := LocGetProc('jpeg_input_complete');
  @jpeg_new_colormap := LocGetProc('jpeg_new_colormap');
  @jpeg_consume_input := LocGetProc('jpeg_consume_input');
  @jpeg_calc_output_dimensions := LocGetProc('jpeg_calc_output_dimensions');
  @jpeg_save_markers := LocGetProc('jpeg_save_markers');
  @jpeg_set_marker_processor := LocGetProc('jpeg_set_marker_processor');
  @jpeg_read_coefficients := LocGetProc('jpeg_read_coefficients');
  @jpeg_write_coefficients := LocGetProc('jpeg_write_coefficients');
  @jpeg_copy_critical_parameters := LocGetProc('jpeg_copy_critical_parameters');
  @jpeg_abort_compress := LocGetProc('jpeg_abort_compress');
  @jpeg_abort_decompress := LocGetProc('jpeg_abort_decompress');
  @jpeg_destroy := LocGetProc('jpeg_destroy');
*)

  if not vAllFound then
    quit_libJPEG;
