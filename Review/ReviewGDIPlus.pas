...
//        GDICheck(vImage.GetLastStatus);
        ...
        vBakName := GetUniqName(FSrcName, '~' + ExtractFileExtension(FSrcName) );

        ApiCheck(RenameFile(FSrcName, vBakName));
        ApiCheck(RenameFile(vNewName, FSrcName));
        if optKeepDateOnSave {soKeepDate in aOptions} then
          SetFileTimeName(FSrcName, FileAge(vBakName));
        DeleteFile(vBakName);
