...
Ограничения:
 ...
 1. Макросы из FML файлов не могут быть вызваны через макрофункцию Eval.

 2. Особенности поведения функций AKey()/$AKey. Начиная с версии 1.0.8 эти
    функции корректно работают при запуске макроса нажатием горячей клавиши 
    или кнопки мыши. Если макрос запускается через меню, они возвращают 0.
    Макроподстановка #AKey, которая использовалась ранее для их замены,
...
Доступные команды:
 ...
 Call ИмяМакроса
   - Вызвать макрос по имени. Если имеется несколько одноименных маросов,
...
функцию CallPlugin. Командная строка передается вторым параметром функции,
при этом писать "FML:" _не_надо_. Пример:

  CallPlugin(MacroLib, "call Hello")

Примечание: Хотя вызов макроса из макроса через CallPlugin похож на вызов
...
Параметры (некоторые параметры имеют несколько синонимов):
 ...
 Area, Areas
   - Макро область. Допустимо указание нескольких областей, через пробел.
     ...
     Для диалогов, не снабженных GUID возможен альтернативный способ
     привязки - по заголовку. Заголовок можно указать непосредственно
     в строке, заключив его в кавычки, или с помощью константы. При 
     указании заголовка допустимо использование масок "*" и "?". Пример:

     Area="Dialog.'Macroses*'" 

     Для макрообластей Editor и Viewer возможна привязка к определенному
     типу файлов. Типы определяются по маске, которая задается аналогично
     заголовкам диалогов. Пример:
 ...
 Bind, Key, Keys, Hotkey, Hotkeys
   - Горячая клавиша для запуска, в формате FAR, например: "CtrlAltF12"
     ...
     Для каждой горячей клавиши может быть указан модификатор, отделенный
     символом ":". Список возможных модификаторов:

      Single  - Макрос срабатывает только при однократном нажатии клавиши
                или кнопки мыши
      Double  - Макрос срабатывает при двойном нажатии клавиши или кнопки
                мыши
      Hold    - Макрос срабатывает при удержании клавиши в течении N мс.
                Период настраивается в реестре. Для кнопок мыши - игнорируется
 ...
 Priority=N, по умолчанию 0
   - Приоритет макроса. Если под условия срабатывания попало сразу несколько 
     макросов, то будет исполнен макрос, обладающий наибольшим приоритетом.
     Если имеется несколько макросов с одинаковым приоритетом, то будет
     показан диалог со списком макросов и предложено выбрать один из них.
 ...
 RunOnRelease=0/1, по умолчанию 0
   - Если = 1, то макрос будет исполняться не при нажатии, а при отпускании
     ...
     Опция оставлена для совместимости. Вместо нее рекомендуется пользоваться
     модификатором клавиш "Release".
