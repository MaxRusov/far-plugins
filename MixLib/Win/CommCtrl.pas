...
uses MixTypes, Messages, Windows, ActiveX;
...
type
  ...
  PPropSheetPage = {$ifdef bUnicode}PPropSheetPageW{$else}PPropSheetPageA{$endif bUnicode};

  LPFNPSPCALLBACKA = function(Wnd: HWnd; Msg: Integer; PPSP: PPropSheetPageA): Integer stdcall;
  LPFNPSPCALLBACKW = function(Wnd: HWnd; Msg: Integer; PPSP: PPropSheetPageW): Integer stdcall;
  LPFNPSPCALLBACK = {$ifdef bUnicode}LPFNPSPCALLBACKW{$else}LPFNPSPCALLBACKA{$endif bUnicode};

  TFNPSPCallbackA = LPFNPSPCALLBACKA;
  TFNPSPCallbackW = LPFNPSPCALLBACKW;
  TFNPSPCallback = {$ifdef bUnicode}LPFNPSPCALLBACKW{$else}LPFNPSPCALLBACKA{$endif bUnicode};
  ...
  _PROPSHEETPAGE = {$ifdef bUnicode}_PROPSHEETPAGEW{$else}_PROPSHEETPAGEA{$endif bUnicode};

  TPropSheetPageA = _PROPSHEETPAGEA;
  TPropSheetPageW = _PROPSHEETPAGEW;
  TPropSheetPage = {$ifdef bUnicode}TPropSheetPageW{$else}TPropSheetPageA{$endif bUnicode};

  PROPSHEETPAGEA = _PROPSHEETPAGEA;
  PROPSHEETPAGEW = _PROPSHEETPAGEW;
  PROPSHEETPAGE = {$ifdef bUnicode}PROPSHEETPAGEW{$else}PROPSHEETPAGEA{$endif bUnicode};
...
function ImageList_LoadImageW(Instance: THandle; Bmp: PWideChar; CX, Grow: Integer;
  Mask: TColorRef; pType, Flags: Cardinal): HIMAGELIST; stdcall;
function ImageList_LoadImage(Instance: THandle; Bmp: PTChar; CX, Grow: Integer;
  Mask: TColorRef; pType, Flags: Cardinal): HIMAGELIST; stdcall;
...
function ImageList_ExtractIcon(Instance: THandle; ImageList: HIMAGELIST;
  Image: Integer): HIcon;
function ImageList_LoadBitmap(Instance: THandle; Bmp: PTChar;
...
type
  PHDItemA = ^THDItemA;
  PHDItemW = ^THDItemW;
  PHDItem = {$ifdef bUnicode}PHDItemW{$else}PHDItemA{$endif bUnicode};
  ...
  _HD_ITEM = {$ifdef bUnicode}_HD_ITEMW{$else}_HD_ITEMA{$endif bUnicode};

  THDItemA = _HD_ITEMA;
  THDItemW = _HD_ITEMW;
  THDItem = {$ifdef bUnicode}THDItemW{$else}THDItemA{$endif bUnicode};

  HD_ITEMA = _HD_ITEMA;
  HD_ITEMW = _HD_ITEMW;
  HD_ITEM = {$ifdef bUnicode}HD_ITEMW{$else}HD_ITEMA{$endif bUnicode};
...
type
  ...
  TBBUTTONINFO = {$ifdef bUnicode}TBBUTTONINFOW{$else}TBBUTTONINFOA{$endif bUnicode};

  PTBButtonInfoA = ^TTBButtonInfoA;
  PTBButtonInfoW = ^TTBButtonInfoW;
  PTBButtonInfo = {$ifdef bUnicode}PTBButtonInfoW{$else}PTBButtonInfoA{$endif bUnicode};

  TTBButtonInfoA = TBBUTTONINFOA;
  TTBButtonInfoW = TBBUTTONINFOW;
  TTBButtonInfo = {$ifdef bUnicode}TTBButtonInfoW{$else}TTBButtonInfoA{$endif bUnicode};
...
const
  ...
  TB_GETBUTTONINFO         = {$ifdef bUnicode}TB_GETBUTTONINFOW{$else}TB_GETBUTTONINFOA{$endif bUnicode};
  TB_SETBUTTONINFO         = {$ifdef bUnicode}TB_SETBUTTONINFOW{$else}TB_SETBUTTONINFOA{$endif bUnicode};
...
procedure DrawStatusTextW(hDC: HDC; lprc: PRect; pzsText: PWideChar;
  uFlags: UINT); stdcall;
procedure DrawStatusText(hDC: HDC; lprc: PRect; pzsText: PTChar;
  uFlags: UINT); stdcall;
...
function CreateStatusWindowW(Style: Longint; lpszText: PWideChar;
  hwndParent: HWND; wID: UINT): HWND; stdcall;
function CreateStatusWindow(Style: Longint; lpszText: PTChar;
  hwndParent: HWND; wID: UINT): HWND; stdcall;
...
type
  ...
  TLVItemA = tagLVITEMA;
  TLVItemW = tagLVITEMW;
  TLVItem = {$ifdef bUnicode}TLVItemW{$else}TLVItemA{$endif bUnicode};
  LV_ITEMA = tagLVITEMA;
  LV_ITEMW = tagLVITEMW;
  LV_ITEM = LV_ITEMA;
...
const
  ...
  LPSTR_TEXTCALLBACK = {$ifdef bUnicode}LPSTR_TEXTCALLBACKW{$else}LPSTR_TEXTCALLBACKA{$endif bUnicode};
...
const
  ...
  LVM_GETITEM            = {$ifdef bUnicode}LVM_GETITEMW{$else}LVM_GETITEMA{$endif bUnicode};
  LVM_SETITEM            = {$ifdef bUnicode}LVM_SETITEMW{$else}LVM_SETITEMA{$endif bUnicode};
  LVM_INSERTITEM         = {$ifdef bUnicode}LVM_INSERTITEMW{$else}LVM_INSERTITEMA{$endif bUnicode};
...
type
  ...
  PLVColumn = {$ifdef bUnicode}PLVColumnW{$else}PLVColumnA{$endif bUnicode};
  ...
  tagLVCOLUMN = {$ifdef bUnicode}tagLVCOLUMNW{$else}tagLVCOLUMNA{$endif bUnicode};

  _LV_COLUMNA = tagLVCOLUMNA;
  _LV_COLUMNW = tagLVCOLUMNW;
  _LV_COLUMN = {$ifdef bUnicode}_LV_COLUMNW{$else}_LV_COLUMNA{$endif bUnicode};

  TLVColumnA = tagLVCOLUMNA;
  TLVColumnW = tagLVCOLUMNW;
  TLVColumn = {$ifdef bUnicode}TLVColumnW{$else}TLVColumnA{$endif bUnicode};

  LV_COLUMNA = tagLVCOLUMNA;
  LV_COLUMNW = tagLVCOLUMNW;
  LV_COLUMN = {$ifdef bUnicode}LV_COLUMNW{$else}LV_COLUMNA{$endif bUnicode};
...
const
  ...
  LVM_GETCOLUMN           = {$ifdef bUnicode}LVM_GETCOLUMNW{$else}LVM_GETCOLUMNA{$endif bUnicode};
...
const
  ...
  LVM_SETCOLUMN           = {$ifdef bUnicode}LVM_SETCOLUMNW{$else}LVM_SETCOLUMNA{$endif bUnicode};
...
const
  ...
  LVM_INSERTCOLUMN        = {$ifdef bUnicode} LVM_INSERTCOLUMNW{$else} LVM_INSERTCOLUMNA{$endif bUnicode};
...
const
  ...
  LVM_GETITEMTEXT         = {$ifdef bUnicode}LVM_GETITEMTEXTW{$else}LVM_GETITEMTEXTA{$endif bUnicode};
...
function ListView_GetItemText(hwndLV: HWND; i, iSubItem: Integer;
  pszText: PTChar; cchTextMax: Integer): Integer;
...
const
  ...
  LVN_GETDISPINFO        = {$ifdef bUnicode}LVN_GETDISPINFOW{$else}LVN_GETDISPINFOA{$endif bUnicode};
  LVN_SETDISPINFO        = {$ifdef bUnicode}LVN_SETDISPINFOW{$else}LVN_SETDISPINFOA{$endif bUnicode};
...
type
  ...
  PLVDispInfo = {$ifdef bUnicode}PLVDispInfoW{$else}PLVDispInfoA{$endif bUnicode};
  ...
  TLVDispInfo = {$ifdef bUnicode}TLVDispInfoW{$else}TLVDispInfoA{$endif bUnicode};

  LV_DISPINFOA = tagLVDISPINFO;
  LV_DISPINFOW = tagLVDISPINFOW;
  LV_DISPINFO = {$ifdef bUnicode}LV_DISPINFOW{$else}LV_DISPINFOA{$endif bUnicode};
...
type
  PTVItemA = ^TTVItemA;
  PTVItemW = ^TTVItemW;
  PTVItem = {$ifdef bUnicode}PTVItemW{$else}PTVItemA{$endif bUnicode};
  ...
  tagTVITEM = {$ifdef bUnicode}tagTVITEMW{$else}tagTVITEMA{$endif bUnicode};

  _TV_ITEMA = tagTVITEMA;
  _TV_ITEMW = tagTVITEMW;
  _TV_ITEM = {$ifdef bUnicode}_TV_ITEMW{$else}_TV_ITEMA{$endif bUnicode};

  TTVItemA = tagTVITEMA;
  TTVItemW = tagTVITEMW;
  TTVItem = {$ifdef bUnicode}TTVItemW{$else}TTVItemA{$endif bUnicode};

  TV_ITEMA = tagTVITEMA;
  TV_ITEMW = tagTVITEMW;
  TV_ITEM = {$ifdef bUnicode}TV_ITEMW{$else}TV_ITEMA{$endif bUnicode};
  ...
  tagTVITEMEX = {$ifdef bUnicode}tagTVITEMEXW{$else}tagTVITEMEXA{$endif bUnicode};

  PTVItemExA = ^TTVItemExA;
  PTVItemExW = ^TTVItemExW;
  PTVItemEx = {$ifdef bUnicode}PTVItemExW{$else}PTVItemExA{$endif bUnicode};

  TTVItemExA = tagTVITEMEXA;
  TTVItemExW = tagTVITEMEXW;
  TTVItemEx = {$ifdef bUnicode}TTVItemExW{$else}TTVItemExA{$endif bUnicode};
...
type
  ...
  PTVInsertStruct = {$ifdef bUnicode}PTVInsertStructW{$else}PTVInsertStructA{$endif bUnicode};
  ...
  tagTVINSERTSTRUCT = {$ifdef bUnicode}tagTVINSERTSTRUCTW{$else}tagTVINSERTSTRUCTA{$endif bUnicode};

  _TV_INSERTSTRUCTA = tagTVINSERTSTRUCTA;
  _TV_INSERTSTRUCTW = tagTVINSERTSTRUCTW;
  _TV_INSERTSTRUCT = {$ifdef bUnicode}_TV_INSERTSTRUCTW{$else}_TV_INSERTSTRUCTA{$endif bUnicode};

  TTVInsertStructA = tagTVINSERTSTRUCTA;
  TTVInsertStructW = tagTVINSERTSTRUCTW;
  TTVInsertStruct = {$ifdef bUnicode}TTVInsertStructW{$else}TTVInsertStructA{$endif bUnicode};

  TV_INSERTSTRUCTA = tagTVINSERTSTRUCTA;
  TV_INSERTSTRUCTW = tagTVINSERTSTRUCTW;
  TV_INSERTSTRUCT = {$ifdef bUnicode}TV_INSERTSTRUCTW{$else}TV_INSERTSTRUCTA{$endif bUnicode};

const
  TVM_INSERTITEMA          = TV_FIRST + 0;
  TVM_INSERTITEMW          = TV_FIRST + 50;
  TVM_INSERTITEM          = {$ifdef bUnicode}TVM_INSERTITEMW{$else}TVM_INSERTITEMA{$endif bUnicode};
...
const
  ...
  TVM_GETITEM             = {$ifdef bUnicode}TVM_GETITEMW{$else}TVM_GETITEMA{$endif bUnicode};
...
const
  ...
  TVM_SETITEM             = {$ifdef bUnicode}TVM_SETITEMW{$else}TVM_SETITEMA{$endif bUnicode};
...
const
  ...
  TVM_EDITLABEL           = {$ifdef bUnicode}TVM_EDITLABELW{$else}TVM_EDITLABELA{$endif bUnicode};
...
const
  ...
  TVM_GETISEARCHSTRING    = {$ifdef bUnicode}TVM_GETISEARCHSTRINGW{$else}TVM_GETISEARCHSTRINGA{$endif bUnicode};
...
type
  ...
  PNMTreeView = {$ifdef bUnicode}PNMTreeViewW{$else}PNMTreeViewA{$endif bUnicode};
  ...
  tagNMTREEVIEW = {$ifdef bUnicode}tagNMTREEVIEWW{$else}tagNMTREEVIEWA{$endif bUnicode};

  _NM_TREEVIEWA = tagNMTREEVIEWA;
  _NM_TREEVIEWW = tagNMTREEVIEWW;
  _NM_TREEVIEW = {$ifdef bUnicode}_NM_TREEVIEWW{$else}_NM_TREEVIEWA{$endif bUnicode};

  TNMTreeViewA  = tagNMTREEVIEWA;
  TNMTreeViewW  = tagNMTREEVIEWW;
  TNMTreeView = {$ifdef bUnicode}TNMTreeViewW{$else}TNMTreeViewA{$endif bUnicode};

  NM_TREEVIEWA  = tagNMTREEVIEWA;
  NM_TREEVIEWW  = tagNMTREEVIEWW;
  NM_TREEVIEW = {$ifdef bUnicode}NM_TREEVIEWW{$else}NM_TREEVIEWA{$endif bUnicode};
...
const
  ...
  TVN_SELCHANGING         = {$ifdef bUnicode}TVN_SELCHANGINGW{$else}TVN_SELCHANGINGA{$endif bUnicode};
  TVN_SELCHANGED          = {$ifdef bUnicode}TVN_SELCHANGEDW{$else}TVN_SELCHANGEDA{$endif bUnicode};
...
const
  ...
  TVN_GETDISPINFO         = {$ifdef bUnicode}TVN_GETDISPINFOW{$else}TVN_GETDISPINFOA{$endif bUnicode};
  TVN_SETDISPINFO         = {$ifdef bUnicode}TVN_SETDISPINFOW{$else}TVN_SETDISPINFOA{$endif bUnicode};
...
type
  ...
  PTVDispInfo = {$ifdef bUnicode}PTVDispInfoW{$else}PTVDispInfoA{$endif bUnicode};
  ...
  tagTVDISPINFO = {$ifdef bUnicode}tagTVDISPINFOW{$else}tagTVDISPINFOA{$endif bUnicode};

  _TV_DISPINFOA = tagTVDISPINFOA;
  _TV_DISPINFOW = tagTVDISPINFOW;
  _TV_DISPINFO = {$ifdef bUnicode}_TV_DISPINFOW{$else}_TV_DISPINFOA{$endif bUnicode};

  TTVDispInfoA = tagTVDISPINFOA;
  TTVDispInfoW = tagTVDISPINFOW;
  TTVDispInfo = {$ifdef bUnicode}TTVDispInfoW{$else}TTVDispInfoA{$endif bUnicode};

  TV_DISPINFOA = tagTVDISPINFOA;
  TV_DISPINFOW = tagTVDISPINFOW;
  TV_DISPINFO = {$ifdef bUnicode}TV_DISPINFOW{$else}TV_DISPINFOA{$endif bUnicode};
...
type
  ...
  tagNMTVGETINFOTIP = {$ifdef bUnicode}tagNMTVGETINFOTIPW{$else}tagNMTVGETINFOTIPA{$endif bUnicode};

  PNMTVGetInfoTipA = ^TNMTVGetInfoTipA;
  PNMTVGetInfoTipW = ^TNMTVGetInfoTipW;
  PNMTVGetInfoTip = {$ifdef bUnicode}PNMTVGetInfoTipW{$else}PNMTVGetInfoTipA{$endif bUnicode};

  TNMTVGetInfoTipA = tagNMTVGETINFOTIPA;
  TNMTVGetInfoTipW = tagNMTVGETINFOTIPW;
  TNMTVGetInfoTip = {$ifdef bUnicode}TNMTVGetInfoTipW{$else}TNMTVGetInfoTipA{$endif bUnicode};
...
type
  ...
  PTCItemA = ^TTCItemA;
  PTCItemW = ^TTCItemW;
  PTCItem = {$ifdef bUnicode}PTCItemW{$else}PTCItemA{$endif bUnicode};
  ...
  tagTCITEM = {$ifdef bUnicode}tagTCITEMW{$else}tagTCITEMA{$endif bUnicode};

  _TC_ITEMA = tagTCITEMA;
  _TC_ITEMW = tagTCITEMW;
  _TC_ITEM = {$ifdef bUnicode}_TC_ITEMW{$else}_TC_ITEMA{$endif bUnicode};

  TTCItemA = tagTCITEMA;
  TTCItemW = tagTCITEMW;
  TTCItem = {$ifdef bUnicode}TTCItemW{$else}TTCItemA{$endif bUnicode};

  TC_ITEMA = tagTCITEMA;
  TC_ITEMW = tagTCITEMW;
  TC_ITEM = {$ifdef bUnicode}TC_ITEMW{$else}TC_ITEMA{$endif bUnicode};
...
function Animate_Open(hwnd: HWND; szName: PTChar): BOOL;
function Animate_OpenEx(hwnd: HWND; hInst: HINST; szName: PTChar): BOOL;
...
function CreatePropertySheetPage; external cctrl name 'CreatePropertySheetPage'+_X;
...
function PropertySheet; external cctrl name 'PropertySheet'+_X;
...
function ImageList_LoadImage; external cctrl name 'ImageList_LoadImage'+_X;
...
procedure DrawStatusText; external cctrl name 'DrawStatusText'+_X;
...
function CreateStatusWindow; external cctrl name 'CreateStatusWindow'+_X;
